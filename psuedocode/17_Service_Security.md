# 17 - Service: Security Service

This file provides the pseudocode for the `SecurityService`. This service is responsible for handling security-related operations such as managing encryption keys, encrypting and decrypting sensitive data (like API tokens or configuration values if needed), and potentially password hashing/verification if not handled directly by an `AuthService` or utility functions. It will utilize the security utilities defined in `app/utils/security_utils.py`.

**Target Application File:** `app/services/security_service.py`
**Corresponding Test File (to be generated by LLM):** `tests/unit/services/test_security_service.py`. Tests should cover key loading/generation, encryption/decryption round trips, and password hashing/verification logic within the service context.

## Pseudocode for Application Code (`app/services/security_service.py`)
...
from app.utils.security_utils import (
    ...
    hash_password as util_hash_password,         # Alias to avoid name collision
    verify_password as util_verify_password,     # Alias
    encrypt_data as util_encrypt_data,           # Alias
    decrypt_data as util_decrypt_data            # Alias
)
...
class SecurityService:
    ...
    def _load_or_generate_fernet_key(self) -> None:
        ...
        try:
            if os.path.exists(self.key_file_path):
                ...
                if not self._fernet_key or len(self._fernet_key) < 44: # Basic check for Fernet key format (base64 encoded 32 bytes)
                    logger.warning(f"Key file at {self.key_file_path} seems invalid or empty. Will attempt to regenerate.")
    ...
    def hash_app_password(self, password: str, salt: Optional[bytes] = None) -> Tuple[bytes, bytes]:
        """
        ...
        return util_hash_password(password, salt, iterations=self._password_iterations)

    def verify_app_password(self, password_to_check: str, stored_hash: bytes, salt: bytes) -> bool:
        """
        ...
        """
        logger.debug(f"Verifying password with {self._password_iterations} iterations via SecurityService.")
        return util_verify_password(password_to_check, stored_hash, salt, iterations=self._password_iterations)
```
